...
| M1' | 閘門本體（`checkDependencyCompatibility`）同時檢查同一件事，但**依落差類別決定成敗**：含阻擋性項目才失敗，全屬 minor / patch / 新增時僅警告（FR-011a） | 阻擋性落差混進 `dev`，拖到發版前才爆 |
| M2 | `publish.yml` 在發布前先跑 M1 | 發布的 artifact 與簽入基準線不一致 |
| M3 | 基準線檔案標頭含「由任務產生，請勿手動編輯」，且格式為排序後純文字 | 手動竄改難以隱藏，且 diff 乾淨 |

**M1' 的嚴格度為何與 M1 不同**（2026-08-02，由 T054 端對端驗證補上）：

初版把 M1 原封不動搬進閘門本體，結果是「把 awssdk 前推一個 patch」——也就是 Dependabot
每週送來的那種 PR——必定紅燈，因為基準線檔還沒重新產生。這直接違反 FR-011 與 SC-003，
而諷刺的是，本節否決 Gradle dependency locking 的理由正正就是「會讓每支 bot PR 紅燈」；
若不區分嚴格度，簽入基準線的方案等於原地把同一個坑再挖一次。

嚴格度之所以能放鬆而不損失保障，關鍵在**閘門的比較對象不是這個檔案**：閘門比的是
「merge-base 當時的基準線 vs 現在的實際解析」，另一側本來就是實際解析結果。
基準線過期不會讓閘門漏判，代價只是把一份不真實的檔案留給下一個人。
因此：非阻擋性落差晚一點補，成本僅是後續 PR 多報幾行資訊性變動；
阻擋性落差則必須當場擋下，否則要等到發版前的 M1 才爆——那是最不該被臨時擋住的時刻。
